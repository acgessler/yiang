...
def fuzzy_match(pot, defs):
    ...
    for candidate in list(defs.values()):
...
def header(pot, defs):
    ...
    '''
    try:
        [po] = [ d for d in list(defs.values()) if d.id == 'msgid ""\n' ]
    except ValueError:
...
def match(defs, refs):
    ...
    matches = []
    empty = Msgs('msgid ""\n', 'msgstr ""\n', '', -1, 'str')
    deco = [(r.lno, r) for r in list(refs.values())]
    deco.sort()
...
def obsolete(defs, matches):
    ...
    deco = [ (d.lno, d) for d in list(defs.values()) if
...
def merge(def_file, ref_file, update = True, outfile = '-',
    ...
    if verbose and not __name__ == '__main__':
        print('Merging %s with %s' % (ref_file, def_file), file=sys.stderr)
...
def merge_dir(directory, pot = False, include = [], exclude = [],
    ...
    for po in pos:
        try:
            ...
            if verbose:
                print('%s Not updated.' % err, file=sys.stderr)
            else:
                print('%s %s not updated.' % (err, po), file=sys.stderr)
